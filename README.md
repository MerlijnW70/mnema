...
**recall**, **reinforce** (strengthen a memory the agent actually used), **prune** (forget
faded memories so the store stays bounded), **remember_fact**, **beliefs**, **forget**,
**stats**. The server is thin glue over the library — every guarantee above still holds, and
the memory logic stays mutation-pinned.
