...
impl SealingKey {
    ...
    pub(crate) fn salt_of(blob: &[u8]) -> Result<[u8; SALT_LEN], StoreError> {
        ...
        Ok(salt.try_into().unwrap()) // noha: take_slice just returned exactly SALT_LEN bytes — infallible
    }
...
pub(crate) fn take_u32(buf: &[u8], off: usize) -> Result<(u32, usize), StoreError> {
    ...
    Ok((u32::from_le_bytes(s.try_into().unwrap()), next)) // noha: take_slice just returned exactly 4 bytes — infallible
}

pub(crate) fn take_u64(buf: &[u8], off: usize) -> Result<(u64, usize), StoreError> {
    let (s, next) = take_slice(buf, off, 8)?;
    Ok((u64::from_le_bytes(s.try_into().unwrap()), next)) // noha: take_slice just returned exactly 8 bytes — infallible
}

pub(crate) fn take_f32(buf: &[u8], off: usize) -> Result<(f32, usize), StoreError> {
    let (s, next) = take_slice(buf, off, 4)?;
    Ok((f32::from_le_bytes(s.try_into().unwrap()), next)) // noha: take_slice just returned exactly 4 bytes — infallible
}
